...
const AddMilestoneModal = ({ onClose, childId, onSuccess }) => {
  const { user } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const link = "/api/milestones/add_milestone"
      const response = await fetch(
        'http://169.239.251.102:3341/~anna.kodji/backend/milestones/add_milestone.php',
        {
          method: 'POST',
          headers: {
      ...
      if (data.success) {
        ...
        setErrors({ submit: data.message || 'Error adding milestone' });
      }
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="p-6 border-b border-[#E8D8D0]">
          <h2 className="text-2xl font-poppins font-medium text-[#2C3E50]">Record New Milestone</h2>
